...
if ($stmt->num_rows > 0 && !($username == $_COOKIE["username"])) {
...
$stmt = $conn->prepare("UPDATE `user` SET username = ?, hashed_password = ?, salt = ?, email = ?, phone_number = ?, address = ?, gender = ?, birthday = ?, nationality = ? WHERE user_id = ?");
$stmt->bind_param("sssssssssi", $username, $hashedPassword, $salt, $email, $phoneNumber, $address, $gender, $birthday, $nationality, $userId);
